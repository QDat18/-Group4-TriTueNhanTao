...
export default function RealtimeAttendance() {
  ...
  const [policy, setPolicy] = useState({ work_start_time: '08:00', work_end_time: '17:30', allow_late_minutes: 5, allow_early_minutes: 15 });
    ...
    getSettings().then(res => {
      if (res && !res.error) {
        setPolicy(p => ({
          ...
          allow_late_minutes: res.allow_late_minutes ?? 5,
  ...
  useEffect(() => {
    loadData();
    if (isStreaming) { pollIntervalRef.current = setInterval(loadData, 3500); }
    else { clearInterval(pollIntervalRef.current); }
  ...
  const faceEmpId = currentFace ? (logs.find(l => l.full_name === currentFace.full_name)?.employee_id || null) : null;
  const ringColor = currentFace ? (STATUS_CONFIG[currentFace.status]?.color || '#94a3b8') : '#334155';
  ...
  return (
    <div className="animate-in" style={{ maxWidth: 1280, margin: '0 auto', paddingBottom: '3rem' }}>
      ...
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid var(--br)', paddingBottom: '1.25rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.25rem' }}>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: isStreaming ? '#10b981' : '#64748b', boxShadow: isStreaming ? '0 0 10px #10b981' : 'none', animation: isStreaming ? 'pulse 1.5s infinite' : 'none' }} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', background: 'var(--bg-subtle)', border: '1px solid var(--br)', padding: '0.5rem 0.85rem', borderRadius: '10px', fontSize: '0.78rem', color: 'var(--tx-2)', fontWeight: 600 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', color: 'var(--ac)', fontWeight: 700, marginBottom: '0.15rem' }}>
              <ShieldCheck size={13} /> Cấu hình Ca làm việc
            </div>
            <div>Vào: <strong>{policy.work_start_time}</strong> (+{policy.allow_late_minutes}m) &nbsp;·&nbsp; Ra: <strong>{policy.work_end_time}</strong> (-{policy.allow_early_minutes}m)</div>
          </div>
          <div style={{ background: '#1e293b', color: '#f8fafc', padding: '0.45rem 1rem', borderRadius: '10px', border: '1px solid #334155', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ fontFamily: 'monospace', fontSize: '1.35rem', fontWeight: 800, letterSpacing: '0.05em', lineHeight: 1.1 }}>{timeStr}</div>
            <div style={{ fontSize: '0.62rem', color: '#94a3b8', marginTop: '0.1rem', textTransform: 'uppercase', fontWeight: 700 }}>{dateStr.split(',')[0] || dateStr}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        {[
          { label: 'Đã vào', value: stats.present, sub: `/ ${stats.total} nhân viên`, color: '#10b981', Icon: UserCheck, bgGlow: 'rgba(16,185,129,.08)' },
          { label: 'Vào muộn', value: stats.late, sub: 'lượt đi muộn', color: '#f59e0b', Icon: Clock, bgGlow: 'rgba(245,158,11,.08)' },
          { label: 'Về sớm', value: stats.earlyLeave, sub: 'lượt về sớm', color: '#f97316', Icon: TrendingDown, bgGlow: 'rgba(249,115,22,.08)' },
          { label: 'Vắng mặt', value: stats.absent, sub: 'chưa check-in', color: '#ef4444', Icon: UserX, bgGlow: 'rgba(239,68,68,.08)' },
          { label: 'Chuyên cần', value: `${stats.rate}%`, sub: 'tỷ lệ hôm nay', color: '#3b82f6', Icon: Percent, bgGlow: 'rgba(59,130,246,.08)' },
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', padding: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.85rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <select value={selectedCameraId} onChange={e => { setSelectedCameraId(e.target.value); setStreamError(false); }}
                style={{ fontSize: '0.75rem', padding: '0.25rem 0.6rem', borderRadius: '6px', border: '1px solid var(--br)', background: 'var(--bg-surface)', color: 'var(--tx-1)', fontWeight: 600, outline: 'none' }}>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', background: isStreaming ? '#ecfdf5' : '#f1f5f9', color: isStreaming ? '#059669' : '#64748b', padding: '0.25rem 0.65rem', borderRadius: '50px', fontSize: '0.7rem', fontWeight: 800 }}>
          ...
          {/* Camera viewport */}
          <div style={{ background: '#070a13', borderRadius: '12px', minHeight: 420, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative', border: '1.5px solid #1e293b', boxShadow: '0 10px 30px rgba(0,0,0,.25), inset 0 0 40px rgba(0,0,0,.8)' }}>
            {isStreaming && streamUrl ? (
              <>
                <img src={streamUrl} style={{ width: '100%', height: '100%', minHeight: 420, objectFit: 'contain', zIndex: 1 }} alt="Live Stream Feed" onError={() => { setIsStreaming(false); setStreamError(true); }} />
                <div className="hud-corner tl" />
                <div className="hud-corner tr" />
                <div className="hud-corner bl" />
                <div className="hud-corner br" />
                <div className="scan-laser-line" />
                <div style={{ position: 'absolute', bottom: '1rem', left: '1rem', fontFamily: 'monospace', fontSize: '0.68rem', zIndex: 10, background: 'rgba(9,13,22,0.85)', padding: '0.6rem 0.85rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.08)', color: '#94a3b8', backdropFilter: 'blur(4px)' }}>
                ...
                <div style={{ position: 'absolute', top: '1rem', left: '1rem', zIndex: 10, background: 'rgba(9,13,22,0.85)', padding: '0.35rem 0.65rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', gap: '0.35rem', backdropFilter: 'blur(4px)' }}>
                ...
                {isRecent && latestLog && (
                  <div style={{ position: 'absolute', top: '1rem', right: '1rem', zIndex: 10, background: 'rgba(15,23,42,0.9)', border: `1px solid ${STATUS_CONFIG[latestLog.status]?.border || '#334155'}`, padding: '0.5rem 0.85rem', borderRadius: '8px', fontSize: '0.75rem', fontFamily: 'monospace', color: STATUS_CONFIG[latestLog.status]?.color || '#fff', boxShadow: `0 4px 15px ${STATUS_CONFIG[latestLog.status]?.color}33`, backdropFilter: 'blur(4px)' }}>
                    MATCH: <strong>{latestLog.full_name}</strong>
                  </div>
                )}
              </>
            ) : (
          ...
          <div style={{ marginTop: '0.85rem' }}>
            <button
              ...
              onClick={() => {
                ...
                if (isStreaming) { setIsStreaming(false); setCurrentFace(null); setCurrentFaceTs(null); }
                else { setIsStreaming(true); }
              }}
            >
              {isStreaming ? <><Square size={14} fill="currentColor" /> DỪNG QUÉT CAMERA</> : <><Play size={14} fill="currentColor" /> BẮT ĐẦU QUÉT CAMERA</>}
            </button>
          </div>
        </div>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', minHeight: 495, padding: '1.25rem' }}>
          ...
          {currentFace ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.85rem' }}>
              ...
              {/* Portrait ring */}
              <div style={{ width: 130, height: 130, borderRadius: '50%', overflow: 'hidden', border: `4px solid ${ringColor}`, boxShadow: `0 0 25px ${ringColor}44`, background: 'var(--bg-subtle)', flexShrink: 0, position: 'relative' }}>
                <Portrait empId={faceEmpId} name={currentFace.full_name} size={130} backend={BACKEND} />
                <div style={{ position: 'absolute', bottom: 4, right: 4, width: 22, height: 22, borderRadius: '50%', background: ringColor, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #fff', boxShadow: '0 2px 6px rgba(0,0,0,.15)' }}>
                  <StatusIcon status={currentFace.status} size={10} />
                </div>
              </div>
              ...
              {/* Liveness only */}
              <div style={{ width: '100%', background: 'var(--bg-subtle)', borderRadius: '10px', padding: '0.85rem', border: '1px solid var(--br)', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexGrow: 1, padding: '2rem 1rem' }}>
              ...
              <p style={{ margin: '1rem 0 0.25rem', fontWeight: 700, fontSize: '0.85rem', color: 'var(--tx-2)', letterSpacing: '0.05em' }}>ĐANG QUÉT KHUÔN MẶT...</p>
      ...
      <div className="card" style={{ marginTop: '1.5rem', padding: '1.25rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--br)', paddingBottom: '0.75rem', marginBottom: '1rem' }}>
          <span style={{ fontSize: '0.78rem', fontWeight: 800, color: 'var(--tx-2)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            <Users size={14} color="var(--ac)" /> Nhật ký điểm danh hôm nay
          </span>
          <button className="btn btn-sm btn-secondary" style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', borderRadius: '6px' }} onClick={loadData}>
                  ...
                  return (
                    <tr key={i} style={i === 0 && isRecent ? { background: `${cfg.bg}aa` } : {}}>
                      <td style={{ textAlign: 'center', padding: '0.45rem' }}>
                        <div style={{ width: 34, height: 34, borderRadius: '50%', overflow: 'hidden', border: `2px solid ${cfg.border}`, background: 'var(--bg-subtle)', margin: '0 auto', boxShadow: i === 0 && isRecent ? `0 0 10px ${cfg.border}` : 'none' }}>
                          <Portrait empId={log.employee_id} name={log.full_name} size={34} backend={BACKEND} />
                        </div>
                      </td>
                      <td style={{ fontFamily: 'monospace', fontSize: '0.85rem', fontWeight: 800, color: 'var(--tx-1)' }}>{formatTime(log.check_time)}</td>
                      <td style={{ fontSize: '0.78rem', color: 'var(--tx-3)' }}>{formatDate(log.check_time)}</td>
                      <td><strong style={{ color: 'var(--ac)', fontSize: '0.82rem', fontFamily: 'monospace' }}>{log.employee_id}</strong></td>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.7rem', fontWeight: 800, color: isCheckOut ? '#3b82f6' : '#10b981' }}>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        {liv !== null ? (
                          <span style={{ fontFamily: 'monospace', fontWeight: 800, fontSize: '0.82rem', color: liv >= 60 ? '#10b981' : '#ef4444' }}>{liv}%</span>
                        ) : '—'}
                      </td>
                      <td style={{ textAlign: 'center' }}>
