...
import { TrendingUp, Users, Clock, UserX, BarChart3, Building2 } from 'lucide-react';
...
const DEPT_COLORS = ['#2563eb', '#16a34a', '#d97706', '#dc2626', '#7c3aed', '#0891b2'];
...
export default function Reports() {
  ...
  const filteredDeptData = useMemo(() => {
    if (!selectedDept) return deptData;
    return deptData.filter(d => d.department === selectedDept);
  }, [deptData, selectedDept]);

  // Selected dept stats
  const selectedDeptStats = useMemo(() => {
    if (!selectedDept) return null;
    return deptData.find(d => d.department === selectedDept) || null;
  }, [deptData, selectedDept]);
  ...
  const summaryCards = summary ? [
    ...
    { label: 'Tỷ Lệ Vắng Mặt', value: `${summary.absent_rate}%`, sub: `${summary.total_employees} nhân viên`, color: '#dc2626', bg: '#fef2f2', icon: UserX },
  ] : [];

  // Pie data for selected dept
  const pieDeptData = selectedDeptStats ? [
    { name: 'Đi làm', value: selectedDeptStats.present, color: '#22c55e' },
    { name: 'Vắng mặt', value: selectedDeptStats.absent, color: '#f87171' },
  ] : [];
  ...
  return (
    <div className="animate-in">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ fontSize: '0.875rem', color: '#9ca3af', marginTop: '0.25rem' }}>
            Thống kê chấm công theo {periodLabel.toLowerCase()}
            {selectedDept && <span style={{ color: '#2563eb', fontWeight: 600 }}> · Phòng ban: {selectedDept}</span>}
          </p>
        </div>
        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          {/* Dept filter */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: '0.3rem 0.5rem 0.3rem 0.75rem' }}>
            <Building2 size={14} color="#6b7280" />
            <select value={selectedDept} onChange={e => setSelectedDept(e.target.value)}
              style={{ border: 'none', background: 'transparent', fontSize: '0.8rem', fontWeight: 600, color: '#374151', outline: 'none', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              <option value="">Tất cả phòng ban</option>
              {deptData.map(d => <option key={d.department} value={d.department}>{d.department}</option>)}
            </select>
          </div>
          {/* Period tabs */}
      ...
      {/* Summary cards — show selected dept stats if filtered */}
      {selectedDeptStats ? (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.375rem', marginBottom: '1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
          <div style={{ fontSize: '0.875rem', fontWeight: 700, color: '#111827', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Building2 size={16} color="#2563eb" /> Thống kê phòng ban: <span style={{ color: '#2563eb' }}>{selectedDept}</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem' }}>
            {[
              { label: 'Tổng NV', value: selectedDeptStats.total, color: '#2563eb', bg: '#eff6ff' },
              { label: 'Đi làm', value: selectedDeptStats.present, color: '#16a34a', bg: '#f0fdf4' },
              { label: 'Vắng mặt', value: selectedDeptStats.absent, color: '#dc2626', bg: '#fef2f2' },
              { label: 'Tỷ lệ', value: `${selectedDeptStats.rate}%`, color: selectedDeptStats.rate >= 90 ? '#16a34a' : selectedDeptStats.rate >= 70 ? '#d97706' : '#dc2626', bg: '#f9fafb' },
            ].map(s => (
              <div key={s.label} style={{ background: s.bg, borderRadius: 12, padding: '1rem', textAlign: 'center' }}>
                <div style={{ fontSize: '1.75rem', fontWeight: 800, color: s.color, letterSpacing: '-0.03em' }}>{s.value}</div>
        ...
        loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem', marginBottom: '1.25rem' }}>
            {summaryCards.map(s => (
              <div key={s.label} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.375rem', position: 'relative', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,.06)', transition: 'transform 180ms ease, box-shadow 180ms ease' }}
      ...
      <div className="grid-2" style={{ marginBottom: '1.25rem' }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.375rem', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.125rem' }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><TrendingUp size={16} color="#2563eb" /></div>
            <div>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={summary.daily_data} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                  ...
                  <Bar dataKey="present" name="Đi làm" radius={[6, 6, 0, 0]}>
                    {summary.daily_data.map((_, i) => <Cell key={i} fill={`hsl(${220 + i * 2}, 80%, ${55 + (i % 3) * 5}%)`} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        {/* Dept chart or Pie if filtered */}
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.375rem', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.125rem' }}>
            <div style={{ width: 32, height: 32, borderRadius: 8, background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><BarChart3 size={16} color="#16a34a" /></div>
            <div>
              <div style={{ fontSize: '0.875rem', fontWeight: 700, color: '#111827' }}>
                {selectedDept ? `Chi tiết: ${selectedDept}` : 'Theo Phòng Ban'}
              </div>
          ...
          {loading ? <div className="skeleton" style={{ height: 240, borderRadius: 8 }} /> :
            selectedDeptStats ? (
              <ResponsiveContainer width="100%" height={240}>
                <PieChart>
                  <Pie data={pieDeptData} cx="50%" cy="50%" outerRadius={90} dataKey="value" label={({ name, value }) => `${name}: ${value}`}>
                    {pieDeptData.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                  </Pie>
                  <Tooltip />
                  <Legend />
                </PieChart>
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={filteredDeptData} layout="vertical" margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
      ...
      {/* Dept detail table */}
      {!loading && filteredDeptData.length > 0 && (
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 16, padding: '1.375rem', boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.125rem' }}>
            ...
            {selectedDept && (
              <button onClick={() => setSelectedDept('')} style={{ border: 'none', background: '#f3f4f6', borderRadius: 8, padding: '0.35rem 0.75rem', fontSize: '0.75rem', fontWeight: 600, color: '#6b7280', cursor: 'pointer' }}>
                ✕ Bỏ lọc
              </button>
            )}
          </div>
          <table className="data-table">
            ...
            <tbody>
              {[...filteredDeptData].sort((a, b) => b.rate - a.rate).map((dept, i) => {
                ...
                return (
                  <tr key={dept.department} style={{ cursor: 'pointer' }} onClick={() => setSelectedDept(dept.department === selectedDept ? '' : dept.department)}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <span>{['🥇', '🥈', '🥉'][i] || `#${i + 1}`}</span>
                        <span style={{ fontWeight: 600 }}>{dept.department}</span>
                        {dept.department === selectedDept && <span style={{ fontSize: '0.68rem', background: '#eff6ff', color: '#2563eb', borderRadius: 99, padding: '0.1rem 0.4rem', fontWeight: 700 }}>Đang lọc</span>}
                      </div>
                    </td>
                    ...
                    <td style={{ textAlign: 'center' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, padding: '0.2rem 0.625rem', borderRadius: 99, background: rateBg, color: rateColor }}>{dept.rate}%</span>
                    </td>
                    <td style={{ width: 140 }}>
