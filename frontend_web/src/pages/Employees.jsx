  ...
  return (
    <div className="animate-in">
      ...
      {showModal && (
        ...
        >
          {/* centering wrapper — takes full overlay height so modal centers */}
          <div style={{
            minHeight:'100%',
            display:'flex',
            alignItems:'center',
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            <div style={{ padding:'1rem 1.25rem 0', flexShrink:0 }}>
              ...
              {/* Avatar row — compact */}
              <div style={{ display:'flex', alignItems:'center', gap:'0.875rem', padding:'0.625rem', background:'#f8fafc', borderRadius:10, border:'1px solid #e5e7eb', marginBottom:'0.75rem' }}>
                <div style={{ width:44, height:44, borderRadius:'50%', overflow:'hidden', border:'2px solid #dbeafe', flexShrink:0 }}>
                  <img
                ...
                <div>
                  <div style={{ fontWeight:700, fontSize:'0.875rem', color:'#111827' }}>{form.full_name||'Nhân viên mới'}</div>
                  <div style={{ fontSize:'0.7rem', color:'#9ca3af', marginTop:1 }}>{form.employee_id ? `Mã: ${form.employee_id}` : 'Chưa có mã NV'}</div>
                </div>
              </div>
            </div>

            {/* Scrollable body */}
            <div className='modal-compact' style={{ padding:'0 1.25rem', overflowY:'auto', flex:1 }}>
              <style>{`.modal-compact .input-group{margin-bottom:.625rem}.modal-compact .input{padding:.5rem .75rem;font-size:.825rem}`}</style>
            ...
            {/* Footer — sticky */}
            <div style={{ padding:'0.75rem 1.25rem', borderTop:'1px solid #f3f4f6', display:'flex', gap:'0.625rem', justifyContent:'flex-end', flexShrink:0 }}>
